...
{{-- KODE PHP UNTUK MENYIAPKAN DATA LAMA VARIASI (MOTIF & SIZE) --}}
@php
    $formattedMotifs = [];
    $grouped = $product->variants->groupBy('motif');
    
    foreach ($grouped as $motifName => $variants) {
        $sizes = [];
    ...
    if (empty($formattedMotifs)) {
        $formattedMotifs = [['name' => '', 'sizes' => [['size' => '', 'price' => '', 'stock' => 0]]]];
    }
@endphp

<div class="container mx-auto p-6" x-data="{ 
    motifs: {{ json_encode(old('motifs', $formattedMotifs)) }}
}">
    <div class="max-w-5xl mx-auto bg-white rounded-[40px] shadow-xl overflow-hidden border border-gray-100">
        ...
        <div class="bg-[#1a1a2e] p-8">
            ...
            <p class="text-gray-400 text-xs mt-1">Perbarui informasi dasar dan variasi stok produk Anda. Tanda (<span class="text-red-500 font-bold">*</span>) wajib diisi.</p>
        </div>
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="p-8 lg:p-12 space-y-10">
            ...
            {{-- Menangkap halaman asal jika dikirim via URL --}}
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <div class="relative" x-data="{
                            ...
                            {{-- Peringatan Dinamis Harga Utama --}}
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" 
                              class="w-full bg-gray-50 border-gray-200 rounded-2xl py-3.5 px-5 focus:border-[#e8c9a0] focus:ring-[#e8c9a0] transition-all" 
                              required placeholder="Tuliskan detail bahan, kenyamanan, dan motif batik...">{{ old('description', $product->description) }}</textarea>
                        ...
                        <div class="p-8 bg-gray-50/50 rounded-[40px] border border-gray-100 relative shadow-sm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                                <div>
                                    ...
                                    <input type="text" :name="`motifs[${mIndex}][name]`" x-model="motif.name" required
                                ...
                                <div>
                                    <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Ubah Foto Motif (Opsional)</label>
                                    ...
                                    <p class="text-[10px] text-gray-400 mt-2 italic ml-1">*Biarkan kosong jika tidak ingin mengubah foto motif.</p>
                                </div>
                            </div>
                                    ...
                                    <template x-for="(sItem, sIndex) in motif.sizes" :key="sIndex">
                                        <div class="grid grid-cols-4 gap-4 items-end bg-gray-50/50 p-4 rounded-2xl border border-dashed border-gray-200">
                                            ...
                                            <div>
                                                ...
                                                {{-- Input tipe text untuk stok, dibatasi 3 digit via @input dan maxlength HTML --}}
            ...
            <div class="space-y-4">
                ...
                <div class="bg-gray-50/50 p-8 rounded-[40px] border border-gray-100">
                    <label class="block text-xs font-bold text-gray-700 uppercase mb-4 ml-1">Tambah Foto Galeri Baru (Opsional)</label>
                    ...
                    <p class="text-[10px] text-gray-400 mt-3 italic ml-1">*Pilih foto baru jika ingin menambahkan koleksi galeri. Foto lama tetap tersimpan.</p>
                </div>
            </div>
